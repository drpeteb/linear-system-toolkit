...
\\
\den(\lgtm | \lgtv) &=& (2\pi)^{-\frac{\lsd^2}{2}} \determ{\lgtv}^{-\frac{\lsd}{2}} \determ{\priorcolumnvariance}^{-\frac{\lsd}{2}} \nonumber \\
...
The solution we propose here is to use Metropolis-Hastings steps targeting the marginal to ensure irreducibility of the sampler, combined with constrained Gibbs sampling steps, which can only modify a component of the parameters, to accelerate the mixing of the chain. As in the full rank case, the target distribution will be the posterior over parameters and the state trajectory $\postden(\lgtm, \lgtv, \ls{1:\timax})$. This means that each of the Metropolis-Hastings steps targeting the marginal posterior $\postden(\lgtm, \lgtv)$ must be followed by a draw from the state conditional $\postden(\ls{1:\timax}|\lgtm, \lgtv)$ in order to complete the iteration. The two two steps together comprise a collapsed Gibbs move \cite{Dyk2008}. Note that the forward-filtering-backward-sampling procedure can be used without modification to draw from the state conditional. See appendix~\ref{app:state-sampling}.
...
\end{IEEEeqnarray}
%
where $\tvfull$ is an $\rk\times\rk$ positive definite matrix and $\tvrot$ is a $\lsd\times\rk$ matrix of orthonormal vectors with a particular structure. The transformation from $\lgtv$ to $\{\tvfull, \tvrot\}$ is also bijective. This factorisation is constructed using Givens rotations and thus we refer to it as the Givens decomposition. A Givens rotation matrix has the following structure,
%
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
%
where $\givrot{} \in [-\pi/2,\pi/2]$ is a rotation in the plane of the $i$ and $j$ coordinate directions.

The Givens decomposition is important because $\tvrot$ is uniquely determined by the state trajectory, whereas $\tvfull$ is free to vary. We provide here the algorithm for conducting the Givens decomposition. The computational complexity is $\bigo{\rk\times(\lsd-\rk)}$. (Note that multiplication by a Givens matrix is $\bigo{1}$ due to sparsity.) Details and further discussion is included in appendix~\ref{app:givens-factorisation}.
...
\begin{algorithmic}[1]
 ...
 \ENDFOR
  \FOR{$j = 1,\dots,\rk$}
   \FOR{$i = \lsd,\dots,\rk+1$}
      \STATE Calculate the rotation $\givrot{i,j} = \tan^{-1}(\tvsign_{j,j} / \tvsign_{i,j})$.
      \STATE Construct the Givens matrix $\givmat{i}{j}{\givrot{i,j}}$.
...
%
\begin{IEEEeqnarray}{rCl}
 \den(\lgtv|\rk) &=& \frac{ \determ{\priorscalematrix}^{\frac{\rk}{2}} }{ 2^{\half\rk\lsd} \pi^{\half\rk(\lsd-\rk)} \gammafun[r]\left(\frac{\rk}{2}\right) } \nonumber \\
...
In this section we introduce two Metropolis-Hastings kernels which target the marginal posterior $\postden(\lgtm,\lgtv)$. Sampling from each of these should be followed by a draw of the state trajectory from $\postden(\ls{1:\timax}|\lgtm,\lgtv)$. Together, this constitutes a collapsed Gibbs move for the target joint posterior distribution \cite{Dyk2008}.

These moves are needed in order to ensure that the sampler can attain any possible values in the parameter space. However, the resulting Markov chain are liable to mix very poorly, since the proposals are dumb random walks. In the following section, we will describe how to draw from the parameter conditional distribution exactly, which greatly accelerates the mixing of the chain.
...
%
\begin{IEEEeqnarray}{rCl}
 \IEEEeqnarraymulticol{3}{l}{ \den(\ls{1:\timax}|\lgtm, \lgtv) \propto \prod_{t=2}^{\timax} \den(\ls{\ti}|\ls{\ti-1},\lgtm,\lgtv) } \nonumber \\
...
\begin{IEEEeqnarray}{rCl}
 \ppslden{\evec{}}(\evec{\rk+1}|\lgtv) = \begin{cases}
                                          \frac{1}{C_{\rk+1}} & \evec{\rk+1} \in \evecset{\rk+1} \\
                                          0 & \text{ otherwise}    .
                                         \end{cases}
\end{IEEEeqnarray}
%
...
%
\begin{IEEEeqnarray}{rCl}
 C_{\rk+1} &=& \frac{\pi^{\half(\lsd-\rk)}}{\gammafun\left(\frac{\lsd-\rk}{2}\right)}     .
\end{IEEEeqnarray}
%
...
\FOR{each of the Metropolis-Hastings kernels targeting $\postden(\lgtm, \lgtv)$, detailed in sections~\ref{sec:mh-rw-transition-matrix},~\ref{sec:mh-rw-transition-covariance},~and~\ref{sec:rjmcmc-covariance-rank}}
%
\STATE \label{step:mh-start} Propose new parameter values $\lgtm\mcnew, \lgtv\mcnew$.
\STATE With the appropriate acceptance probability, set $\lgtm\mcold \leftarrow \lgtm\mcnew$, $\lgtv\mcold \leftarrow \lgtv\mcnew$.
\STATE \label{step:mh-end} Sample $\ls{1:\timax}\mcold \sim \postden(\ls{1:\timax}|\lgtm\mcold, \lgtv\mcold)$.
%
\STATE \label{step:gibbs-start} Calculate $\tvfull\mcold, \tvrot\mcold$, the Givens decomposition of $\lgtv\mcold$.
...
In the previous sections, we have assumed that $\lgom$ and $\lgov$ are fixed and known a priori. In practice, $\lgov$ will often be unknown. In both the following examples we use models which have,
\begin{IEEEeqnarray}{rCl}
 \lgov &=& \xi_{\ob{}} \idmat     ,
\end{IEEEeqnarray}
%
...
\begin{figure}
 ...
 \caption{MCMC trace plots for $\xi_{\ob{}}$ for \protect\subref{subf:toy-R-trace:basic} the full rank transition model and \protect\subref{subf:toy-R-trace:degenerate} the degenerate transition model.}
...
\begin{figure}
 ...
 \caption{MCMC autocorrelation plots for $\xi_{\ob{}}$ for \protect\subref{subf:toy-R-acf:basic} the full rank transition model and \protect\subref{subf:toy-R-acf:degenerate} the degenerate transition model.}
 \label{fig:toy-R-acf}
\end{figure}
...
%
\begin{IEEEeqnarray}{l}
 \den(\ls{\ti}|\ls{\ti+1}, \ob{1:\ti}, \lgtm, \lgtv) \nonumber \\
 \quad \propto \den(\ls{\ti+1}|\ls{\ti}, \lgtm, \lgtv) \den(\ls{\ti}| \ob{1:\ti}, \lgtm, \lgtv) \nonumber \\
 \quad = \normalden{\ls{\ti}}{ \lsfmn{\ti} + K_t (\ls{\ti+1}-\lgtm\lsfmn{\ti}) }{ \lspvr{\ti} - K_t \lgtm \lsfvr{\ti} }      ,
\end{IEEEeqnarray}
%
...
%
\begin{IEEEeqnarray}{rCl}
\tvvec = \tvsigndiag &\times& \left[ \givmat{1}{2}{\givrot{1,2}}\right] \times \dotsm \nonumber\\
