...
Note that the forward-filtering-backward-sampling procedure can be used as before to draw from the state conditional. See appendix~\ref{app:state-sampling}. The parameter conditional, on the other hand, will require some modifications.
...
The forward-filtering-backward-sampling procedure for drawing from $\postden(\ls{1:\timax}|\lgtm, \lgtv)$ can be applied to a degenerate model without modification. The Kalman filter stage is 
