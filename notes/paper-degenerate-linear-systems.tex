...
%
\begin{IEEEeqnarray}{rCl}
 \den(\lgtv|\rk) &=& \frac{ \determ{\priorscalematrixrank{\rk}}^{\frac{\rk}{2}} }{ 2^{\half\rk\lsd} \pi^{\half\rk(\lsd-\rk)} \gammafun[r]\left(\frac{\rk}{2}\right) } \determ{\tvval}^{-\half(3\lsd-\rk+1)} \exp\left( -\half \lgtv\pinv \priorscalematrixrank{\rk} \right)    ,
\end{IEEEeqnarray}
%
...
%
\begin{IEEEeqnarray}{rCl}
 \beta &=& \frac{ \postden(\lgtm, \lgtv\mcnew, \rk+1) \jac(\lgtv,\eval{\rk+1},\evec{\rk+1}\to\lgtv\mcnew) }{ \postden(\lgtm,\lgtv,\rk) \ppslden{\eval{}}(\eval{\rk+1}|\lgtv) \ppslden{\evec{}}(\evec{\rk+1}|\lgtv) } \times \frac{P(\rk+1\to\rk)}{P(\rk\to\rk+1)}     ,
\end{IEEEeqnarray}
%
...
\begin{figure}
 \centering
 \subfloat[]{\includegraphics[width=0.9\columnwidth]{figures/toy-basic-R-acf.pdf} \label{subf:toy-R-acf:basic}} \\
 \subfloat[]{\includegraphics[width=0.9\columnwidth]{figures/toy-degenerate-R-acf.pdf} \label{subf:toy-R-acf:degenerate}}
...
Markov chain Monte Carlo algorithms are an effective method for Bayesian learning of linear state space systems. In this paper, we have introduced a number of new MCMC kernels for the linear system toolkit.





Although the Markov chain takes substantially longer to converge with the degenerate covariance, it should be noted that the estimated model is better than that obtained in the full rank case after just a few hundred iterations, in the sense that the RMSE for the inferred missing positions is lower. Furthermore, using the degenerate model, we obtain an explicit estimate of the covariance rank. 



\meta{ 
\begin{itemize}
 \item Slow but better model.
 \item Generalises the basic case.
 \item Hyperparameters
 \item Future work: better MH moves.
\end{itemize}
}
...
\section{Conclusions}
 ...
 New parameterisations have enabled us to formulate algorithms for learning transition models when it is known a priori that the model is (i) sparse or (ii) degenerate. The new algorithms are efficient and require minimal tuning, and have been verified with simulations.
